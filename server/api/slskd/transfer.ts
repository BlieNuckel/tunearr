...
export async function enqueueDownload(username: string, files: { filename: string; size: number }[]): Promise<void> {
  const { baseUrl, headers } = getSlskdConfig();

  const response = await fetch(`${baseUrl}/api/v0/transfers/downloads/${encodeURIComponent(username)}`, {
    method: "POST",
    headers,
...
export async function cancelDownload(username: string, id: string): Promise<void> {
  const { baseUrl, headers } = getSlskdConfig();
